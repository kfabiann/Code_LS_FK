...
########################
# Define borders interactively in plot
...
# Kraefte ohne Eingriff (Eigenmasse der Messplattform usw.)
...
for Datei in Dateien:
    ...
    BeginnMittelung = int(Auswertegrenzen[ZeileExcel, 26] / 100 * AnzWerte)
    EndeMittelung = int(
        Auswertegrenzen[ZeileExcel, 27] / 100 * (AnzWerte - 1))
    Wandstaerke = float(Auswertegrenzen[ZeileExcel, 17]) * 1000

    # AvgSchnittkraft = np.mean(Daten[BeginnMittelung:EndeMittelung,2])/Wandstaerke # Falsch! Koordinatensytem der Kraftmeßplattform ist nicht gleich! HK, Fr, 05.06.2020
    # AvgPassivkraft = np.mean(Daten[BeginnMittelung:EndeMittelung,1])/Wandstaerke # x-Achse ist auf Kraftmeßplattform und Maschine gleich! HK, Fr, 05.06.2020
    # AvgVorschubkraft = np.mean(Daten[BeginnMittelung:EndeMittelung,3])/Wandstaerke # Falsch! Koordinatensytem der Kraftmeßplattform ist nicht gleich! HK, Fr, 05.06.2020
    # AvgVorschubkraft = np.mean(Daten[BeginnMittelung:EndeMittelung,2])/Wandstaerke # Korrigierte Zuordnung der Prozeßkräfte, HK, Fr, 05.06.2020
    # AvgSchnittkraft =
    # np.mean(Daten[BeginnMittelung:EndeMittelung,3])/Wandstaerke #
    # Korrigierte Zuordnung der Prozeßkräfte, HK, Fr, 05.06.2020
    # Korrigierte Zuordnung der Prozeßkräfte, HK, Fr, 05.06.2020
    ...
    #
    # Varianz berechnen
    # SigSchnittkraft = np.sqrt(np.var(Daten[BeginnMittelung:EndeMittelung,2]))/Wandstaerke # Falsch! Koordinatensytem der Kraftmeßplattform ist nicht gleich! HK, Fr, 05.06.2020
    # SigPassivkraft = np.sqrt(np.var(Daten[BeginnMittelung:EndeMittelung,3]))/Wandstaerke # x-Achse ist auf Kraftmeßplattform und Maschine gleich! HK, Fr, 05.06.2020
    # SigVorschubkraft = np.sqrt(np.var(Daten[BeginnMittelung:EndeMittelung,3]))/Wandstaerke # Falsch! Koordinatensytem der Kraftmeßplattform ist nicht gleich! HK, Fr, 05.06.2020
    ...
    # Aktuelle Schnittkraft speichern; später in Datei ausschreiben
...
#    plt.plot(Daten[:,0], Daten[:,3]/Wandstaerke, color="blue", linewidth=1.0, linestyle="-", label=r'Schnittkraft [N/mm]') # Korrigierte Zuordnung der Prozeßkräfte, HK, Fr, 05.06.2020
#    #plt.plot(Daten[:,0], Daten[:,1]/Wandstaerke, color="red", linewidth=1.0, linestyle="-", label=r'Passivkraft [N/mm]') # x-Achse ist auf Kraftmeßplattform und Maschine gleich! HK, Fr, 05.06.2020
#    plt.plot(Daten[:,0], Daten[:,2]/Wandstaerke, color="red", linewidth=1.0, linestyle="-", label=r'Vorschubkraft [N/mm]') # Korrigierte Zuordnung der Prozeßkräfte, HK, Fr, 05.06.2020
#
...
#    plt.title(r'Proze{\ss}kraftverlauf, Versuch ' + str(Versuchsnummer) + ', vc= ' + vc + ', f= ' + f)
    ...
    if (DiagrammKraftverlauf != 0):
        ...
        ax.set_title(r'Process forces, Experiment ' +
                     str(Versuchsnummer) + ', $v_c=$ ' + str(vc) + ' [m/min]' + ', $f=$ ' + str(f) + ' [mm]')
        ...
        # ist auf Kraftmeßplattform und Maschine gleich! HK, Fr, 05.06.2020
...
# Auswertung Ruhekr�fte vor der Zerspanung (ab Versuch V0102)
    # HK, Sa, 06.06.2020
    # Vorschub = float(f.split()[0]) # [mm]
    # vSchnitt = float(vc.split()[0])/60.0 # [m/s]
    # D_Zyl = 0.060 # [m] Durchschnittsdurchmesser zur Absch�tzung der Zeit bis zur eigentlichen Zerspanung 1mm Weg mit f bis erster Eingriff (abgerundet, konservativ)
    # n= 1.0/Vorschub # [mm/mm] Umdrehungen bei aktuellem Vorschub f�r 1mm Weg

    # Startzeit f�r Leerlaufkraftermittlung
    tStart = float(Auswertegrenzen[int(Versuchsnummer) - 1, 6])
    # Endzeit f�r Leerlaufkraftermittlung
    tEnde = float(Auswertegrenzen[int(Versuchsnummer) - 1, 7])

    Idx_tStart = 0  # Startindex der Auswertung der Leerlaufkr�fte
    # if (int(Versuchsnummer) == 1): Idx_tStart = np.argmax(Daten[:,0] > 0.25)
    # # im ersten Versuch ab 0,25s Leerlaufkraft auswerten

    # Endindex der Auswertung der Leerlaufkr�fte
    ...
    if (DiagrammLeerlaufkraft != 0):
        ...
        #plt.title(r'Leerlaufkr�fte , Experiment ' + str(Versuchsnummer) + ', vc= ' + vc + ', f= ' + f)
...
plt.title(r'Leerlaufkr�fte, vor eigentlichem Versuch (NAN entfernt); Fc AvgAlle:' +
          str(round(AvgFcLeerAlle, 1)) + 'Ff AvgAlle:' + str(round(AvgFfLeerAlle, 1)))
plt.legend()
#plt.ylim(-3, 1)
...
Datei.write('Gemittelte Proze�kr�fte\n')
Datei.write('Die Leerlaufkr�fte sind schon auf 1mm Schnittbreite bezogen, bed�rfen also keiner Anpassung. Die Standardabweichungen der Proze�kr�fte sind im Prinzip von der Leerlaufkraft unabh�ngig.\n')
